export default function Banner({ modelLoaded, revision }: { modelLoaded: boolean; revision?: string | null }) {
  if (modelLoaded) {
    return (
      <div style={{ background: '#ecfdf5', borderBottom: '1px solid #a7f3d0', padding: '10px 32px', fontSize: '13px', color: '#065f46', display: 'flex', gap: '8px', alignItems: 'center' }}>
        <span>✓ Model loaded</span>
        {revision && <span className="mono" style={{ background: '#fff', padding: '1px 6px', borderRadius: '4px', border: '1px solid #a7f3d0', fontSize: '12px' }}>rev {revision}</span>}
        <span style={{ color: '#047857' }}>— ranking reflects RWR+fusion scores over STRING network.</span>
      </div>
    )
  }
  return (
    <div style={{ background: 'var(--warn-bg)', borderBottom: '1px solid var(--warn-border)', padding: '12px 32px', fontSize: '13px', color: '#92400e', display: 'flex', gap: '10px', alignItems: 'center' }}>
